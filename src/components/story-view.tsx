...
export function StoryView({ story, onBack }: StoryViewProps) {
  const [content, setContent] = useState<(string | { type: 'dialogue'; character: string; line: string })[]>(story.chapters[0].content);
            ...
            <div>
              {content.map((item, index) => {
                if (typeof item === 'string') {
                  return <p key={index} className="text-justify leading-relaxed">{item}</p>;
                }
                return (
                  <div key={index} className="flex items-start gap-4 my-4">
                    <strong className="font-bold text-primary font-headline whitespace-nowrap">{item.character}:</strong>
